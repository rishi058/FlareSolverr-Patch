...
class V1RequestBase(object):
    ...
    # --- request.post2 fields -------------------------------------------------
    # The protected site's home URL (CF challenge is solved here)
    base_url: str = None
    # Path/endpoint to POST to, relative to base_url  e.g. "api/proxy"
    post_endpoint: str = None
    # JSON body for the POST — either a dict or a JSON string
    post_json_body = None
    # -------------------------------------------------------------------------
...
class V1ResponseBase(object):
    ...
    # V1ResponseSolution (used by request.get / request.post)
    solution: ChallengeResolutionResultT = None

    # request.post2 response — status_code + body from the target endpoint
    target_url_response: dict = None
